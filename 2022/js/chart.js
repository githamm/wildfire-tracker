$(document).ready(function() {
    ...
    //$.getJSON("https://services3.arcgis.com/T4QMspbfLg3qTGWY/arcgis/rest/services/CY_WildlandFire_Locations_ToDate/FeatureServer/0/query?where=IncidentTypeCategory%20%3D%20'WF'%20AND%20POOState%20%3D%20'US-CO'&outFields=DailyAcres,FireCause,IncidentName,PercentContained,POOCounty,POOState,IncidentTypeCategory,FireDiscoveryDateTime&outSR=4326&f=json", function(result) {
    $.getJSON('https://raw.githubusercontent.com/githamm/wildfire-data/main/table_data.json', function(result) {
        ...
        for (let i = 0; i < apiData.length; i++) {
            if (apiData[i].attributes.DailyAcres >= 1000) {
                bigFires.push({
                    fire_discovery_date_time: moment(apiData[i].attributes.FireDiscoveryDateTime).format('M/D/YYYY'),
